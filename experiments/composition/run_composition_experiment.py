...
import os 
from implicit_learning.utils import MinMaxScaler
...
class CompositePoissonEqn(Dataset):
    def __init__(self, config, transform, scaler1, scaler2):
        ...
        self.coords = get_mgrid(256, 256, 2)
        self.side_lengths = (256, 256)
...
def construct_dataloader(config):
    scaler1 = MinMaxScaler(1)
    scaler2 = MinMaxScaler(1)
    transform = Compose([
        Resize(256),
        ToTensor(),
...
class RGBTrainer(PlotTrainer):
    def __init__(self,  model, train_dataloader, valid_dataloader, test_dataloader, scaler1, scaler2, config, beta=0.8):
        super().__init__(train_dataloader, valid_dataloader, test_dataloader,  MinMaxScaler(1), config)
