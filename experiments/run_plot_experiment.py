...
class CustomizeTrainer(PoissonTrainer):
    ...
    def train(self):
        ...
        for epoch in range(self.epochs+1):
            ...
            if epoch %  self.plot_epoch == 0:
                ...
                ssim = self.detach_and_calculate_ssim(outputs, self.gt['pixels'])
                ...
                if self.plot_each:
                    self.plot(outputs, self.gt['pixels'], img_grad, self.gt, epoch, hs_image, hs_sidelength)
                if self.plot_full:
                    self.save_for_full_plot(outputs, img_grad, epoch, hs_image, hs_sidelength)
    ...
    def detach_and_calculate_psnr(self, model_output, ground_truth):
        model_output = model_output.view(self.sidelength, self.sidelength).detach().clone().cpu().numpy()
        ground_truth = ground_truth.view(self.sidelength, self.sidelength).detach().clone().cpu().numpy()
    ...
    def save_for_full_plot(self, model_output, img_grad, epoch, high_resolution=None, hs_sidelength=None ):
        self.model_outputs.append(model_output.cpu().view(self.sidelength,self.sidelength).detach().clone().numpy())
        self.model_gradients.append(img_grad.cpu().norm(dim=-1).view(self.sidelength,self.sidelength).detach().clone().numpy())
        self.plot_epochs.append(epoch)
        if self.high_resolution:
            self.high_resolutions.append(high_resolution.cpu().norm(dim=-1).view(hs_sidelength, hs_sidelength).detach().numpy())
    ...
    def store_full_plot(self):
        ...
        for i in range(LENGTH):
            ...
            axes[i,1].imshow(self.gt['pixels'].cpu().view(self.sidelength,self.sidelength).detach().clone().numpy())
            axes[i,2].imshow(self.model_gradients[i])
            axes[i,3].imshow(self.gt['grads'].cpu().norm(dim=-1).view(self.sidelength,self.sidelength).detach().clone().numpy())
        ...
        for i in range(LENGTH):
            ...
            axes[i,0].plot(self.gt['pixels'].cpu().view(self.sidelength,self.sidelength).detach().clone().numpy()[x_line,:])
            axes[i,0].plot(np.cumsum(self.gt['grads'].cpu().view(self.sidelength,self.sidelength,2).detach().clone().numpy()[x_line,:,1]/self.sidelength * 8))
            axes[i,0].set_ylabel(self.plot_epochs[i], rotation=0,fontsize=20)
        
            axes[i,1].plot(self.model_outputs[i][:,y_line])
            axes[i,1].plot(self.gt['pixels'].cpu().view(self.sidelength,self.sidelength).detach().clone().numpy()[:,y_line])
            axes[i,1].plot(np.cumsum(self.gt['grads'].cpu().view(self.sidelength,self.sidelength,2).detach().clone().numpy()[:,y_line,0]/self.sidelength * 8))
        plt.tight_layout()
        ...
        for i in range(LENGTH):
            ...
            axes[i,0].hist(self.gt['pixels'].flatten().cpu().detach().numpy(), bins=200,log=True, alpha=0.5)
            axes[i,1].hist(self.model_gradients[i].flatten(), bins=200,log=True, alpha=0.5)
            axes[i,1].hist(self.gt['grads'].view(1, self.sidelength, self.sidelength,2)[:,:,:,0].flatten().cpu().detach().numpy()*8, bins=200,log=True, alpha=0.5)
            axes[i,0].set_ylabel(self.plot_epochs[i], rotation=0,fontsize=20)
    ...
    def plot(self, model_output, original, img_grad, gt, epoch, high_resolution=None, hs_sidelength=None):
        ...
        if high_resolution is None:
            ...
            axes[4].imshow(high_resolution.cpu().norm(dim=-1).view(hs_sidelength, hs_sidelength).detach().numpy())
            axes[4].set_title("HS X %.1f"%(hs_sidelength/self.sidelength))

        axes[0].imshow(model_output.cpu().view(self.sidelength,self.sidelength).detach().clone().numpy())
        axes[1].imshow(original.cpu().view(self.sidelength,self.sidelength).detach().clone().numpy())
        axes[2].imshow(img_grad.cpu().norm(dim=-1).view(self.sidelength,self.sidelength).detach().clone().numpy())
        axes[3].imshow(gt['grads'].cpu().norm(dim=-1).view(self.sidelength,self.sidelength).detach().clone().numpy())
        ...
        x_line, y_line = 50, 50
        axes[0].plot(model_output.cpu().view(self.sidelength,self.sidelength).detach().clone().numpy()[x_line,:])
        axes[0].plot(original.cpu().view(self.sidelength,self.sidelength).detach().clone().numpy()[x_line,:])
        axes[0].plot(np.cumsum(gt['grads'].cpu().view(self.sidelength,self.sidelength,2).detach().clone().numpy()[x_line,:,1]/self.sidelength * 8))
        axes[0].legend(["model_out", "gt", "cum_sum"])
        axes[0].set_title(f"X_line: {x_line}")

        axes[1].plot(model_output.cpu().view(self.sidelength,self.sidelength).detach().clone().numpy()[:,y_line])
        axes[1].plot(original.cpu().view(self.sidelength,self.sidelength).detach().clone().numpy()[:,y_line])
        axes[1].plot(np.cumsum(gt['grads'].cpu().view(self.sidelength,self.sidelength,2).detach().clone().numpy()[:,y_line,0]/self.sidelength * 8))
        ...
        axes[1].hist(gt['grads'].view(1, self.sidelength, self.sidelength,2)[:,:,:,0].flatten().cpu().detach().numpy()*8, bins=200,log=True, alpha=0.5)
